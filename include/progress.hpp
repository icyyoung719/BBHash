...
private:
	void print_timer_progress(uint64_t current_done)
	{
		...
		double remaining_sec =
		    (speed > 0.0 && todo > current_done) ? (static_cast<double>(todo - current_done) / speed) : 0.0;
		...
		std::fprintf(stderr, "%c[%s]  %-5.3g%%   elapsed: %3i min %-2.0f sec   remaining: %3i min %-2.0f sec", 13,
		             message.c_str(), 100 * (static_cast<double>(current_done) / todo), min_e, sec_e, min_r, sec_r);
	}
};
